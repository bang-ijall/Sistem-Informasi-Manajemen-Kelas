...
export default function KelasPage() {
  ...
  const fetchKelas = async () => {
    try {
      ...
      if (data.error) {
        ...
        setKelas(data.data.map(({ id, ...rest }) => ({ ...rest })));
      }
    } catch (err) {
  ...
  const columns = kelas.length > 0 ? Object.keys(kelas[0]) : [];
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-semibold text-gray-800">Kelas</h2>
        <button
            ...
            {!loading && !error && kelas.map((s) => (
              <tr key={s.kode}>
                ...
                <td className="px-6 py-4 space-x-2">
                  <button
                    onClick={() => router.push(`/kelas/edit?kode=${s.kode}`)}
