...
export default function GuruPage() {
  ...
  const fetchGuru = async () => {
    try {
      ...
      if (data.error) {
        ...
        setGuru(data.data.map(({ id, ...rest }) => ({ ...rest })));
      }
    } catch (err) {
  ...
  const columns = guru.length > 0 ? Object.keys(guru[0]) : [];
  ...
  const handleDelete = async (nisn) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const res = await fetch(`/api/v1/admin/guru/${nisn}`, {
            method: "DELETE",
            headers: { "Content-Type": "application/json" },
          });
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-semibold text-gray-800">Guru</h2>
        <button
          onClick={() => router.push("/guru/edit")}
            ...
            {!loading && !error && guru.map((s) => (
              <tr key={s.nip}>
                ...
                <td className="px-6 py-4 space-x-2">
                  <button
                    onClick={() => router.push(`/guru/edit?nip=${s.nip}`)}
                    className="px-3 py-1 bg-yellow-400 text-white rounded">Edit
                  </button>
                  <button
                    onClick={(e) => handleDelete(s.nip)}
